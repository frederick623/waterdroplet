...
| **Wind slider** | Negative = left wind, positive = right wind. Drives Gerstner waves (still water at 0), rain streak angle, foam, and a slight camera lean. |
...
- Full-screen canvas with an infinite water plane (controls overlay the surface)
- Static surface when calm — motion only from rain ripples or wind-driven waves
- Gerstner waves scaled by wind strength (no perpetual stream drift)
...
- Sparse rain streaks in the air, angled by wind
